        ...
        $hasSrcset.each(function() {
            ...
            const srcset = $this.attr('srcset')
                ...
                .map(candidate => {
                    ...
                    if( (candidateValue === opts.resolutionSuffix && !opts.resolution)
                        || (candidateValue === opts.responsiveSuffix && !opts.responsive) )
                        return null;
            ...
            if(opts.removeSrc) {
                ...
                if(opts.force1xSrc) {
                    if(opts.skip1xSuffix)
                        ...
                        urlObj.pathname = filePath.first + opts.resolutionSuffix.replace('[match]', '1') + filePath.last;

                    $this.attr('src', url.format(urlObj));
                }
            } else if(opts.responsiveSrcReplace >= 0) {
                setSizes($this);
                const suffix = opts.responsiveSuffix.replace('[match]', opts.responsiveWidth[opts.responsiveSrcReplace]);
    ...
    if(opts.baseSrc) {
        $noSrcset.each(function() {
            ...
            if(responsiveMath !== null) {
                ...
                const num = Math.min(Math.max(0, opts.responsiveSrcReplace), widthList.length - 1);
                const suffix = opts.responsiveSuffix.replace('[match]', widthList[num]);
                urlObj.pathname = filePath.first + suffix + filePath.last;
